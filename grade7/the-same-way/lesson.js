...
function render(){const s=SLIDES[i];$('#stage').scrollTop=0;let c='';
...
$('#menuBtn').onclick=()=>$('#menu').showModal();$('#closeMenu').onclick=()=>$('#menu').close();$('#restart').onclick=()=>{state.route=[];state.choices={};$('#menu').close();go(0)};
$('#fullBtn').onclick=()=>document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen?.();
const stops=[{label:'פתיחה',n:0},{label:'הסיפור המלא',n:1},{label:'חיבור ל־To Be ולאוצר המילים',n:4}];for(const p of pmap){stops.push({label:`פסקה ${p}`,n:SLIDES.findIndex(s=>s.kind==='sentence'&&s.part===p)})}stops.push({label:'משימת הדף',n:SLIDES.findIndex(s=>s.kind==='work')},{label:'בדיקת יציאה',n:SLIDES.findIndex(s=>s.kind==='exit')});
