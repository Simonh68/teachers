...
import json,re,sys,threading,http.server,functools,subprocess,time,urllib.request,concurrent.futures
...
if '--live' in sys.argv:
    ...
    for attempt in range(12):
        try:
            status,html,ct=fetch(base+'?release=20260922-unit1')
            assert status==200 and b'unit-data.js' in html
            status,payload,ct=fetch(base+'unit-data.js?release=20260922-unit1')
            assert status==200 and b'2026-10-22' in payload
            break
        except Exception:
            if attempt==11:raise
            time.sleep(10)
    ...
    for name in ['reading/','sentences/','teacher.html']:
        assert fetch(base+name)[0]==200
    record('Public direct links',3)
    print(json.dumps({'live':True,'checks':results},ensure_ascii=False),flush=True)
    (OUT/'live-check.json').write_text(json.dumps({'live':True,'checks':results},ensure_ascii=False,indent=2))
    sys.exit(0)
...
server=http.server.ThreadingHTTPServer(('127.0.0.1',8768),handler);threading.Thread(target=server.serve_forever,daemon=True).start()
base='http://127.0.0.1:8768/grade8/unit-1/'
...
with sync_playwright() as p:
    ...
    for width,height in [(360,640),(390,844),(1365,900)]:
        ...
        assert '22.10.2026' in page.locator('#home').inner_text()
        assert page.locator('.meeting').count()==5
        ...
        for view,count in [('chunks',10),('sentences',36)]:
            for i in range(count):
                ...
                words=page.locator('.word').count()
                ids=data['pages'][i] if view=='chunks' else [i//2]
                assert words==sum(len(data['story'][j]['tokens']) for j in ids)
                for j in ids:
                    assert page.locator(f'[data-text="{j}"]').inner_text()==data['story'][j]['plain']
                if view=='sentences' and i%2==0:
                    before=page.locator('.sentence-text').bounding_box()
                    page.locator('#next').click();after=page.locator('.sentence-text').bounding_box()
        ...
        page.goto(base+'?view=sentences&p=8');page.locator('.word').first.click();assert page.locator('#tip').is_visible();r=page.locator('#tip').bounding_box();assert r['x']>=0 and r['x']+r['width']<=width+1
        page.keyboard.press('Escape');assert page.locator('#tip').is_hidden()
        # Real Chromium touch input, starting on a translatable word.
        ...
        for dx,dy,delta in [(-70,0,1),(70,0,-1),(0,-70,1),(0,70,-1)]:
            ...
            swipe(x,y,dx,dy)
            current=int(page.locator('#counter').inner_text().split('/')[0].strip())-1
            # Vertical gestures in a scrollable area deliberately preserve scrolling.
            if dy and page.evaluate('document.querySelector("#stage").scrollHeight>document.querySelector("#stage").clientHeight+4'):
                continue
    ...
    # Playback on every English and translation slide, with two-second start delay.
    for i in range(36):
        page.goto(base+'?view=sentences&p='+str(i));page.wait_for_selector('.word')
        page.wait_for_timeout(900);assert page.locator('#play').inner_text()=='▶ הקראה'
        page.wait_for_function('!audio.paused && audio.currentTime>0',timeout=10000)
        assert page.evaluate('audio.playbackRate')==.75
        assert page.evaluate('audio.currentTime')>0
        page.evaluate('audio.currentTime=D.story[sentenceIndex].tokens[0].start+0.01')
    ...
    page.goto(base+'?view=sentences&p=8');page.wait_for_selector('.word');page.locator('#next').click();page.locator('#next').click();page.wait_for_timeout(2400);assert page.evaluate('sentenceIndex')==5
    record('Rapid navigation cancels the previous audio timer')
    page.goto(base+'?view=chunks&p=0');page.locator('#play').click();page.wait_for_function('!audio.paused')
    page.evaluate('sentenceIndex=D.pages[pos].at(-1);audio.currentTime=audio.duration-.05')
    page.wait_for_timeout(500);assert page.evaluate('audio.paused');assert page.evaluate('pos')==0
    record('Chunk playback pauses at page boundary')
    page.goto(base);page.locator('[data-tick="m0-done"]').check();page.locator('a[href="?view=chunks"]').first.click();page.goto(base);assert page.locator('#resetHistory').is_enabled();page.locator('#resetHistory').click();assert page.locator('[data-tick="m0-done"]').is_checked();page.locator('#restoreHistory').click();assert page.locator('#resetHistory').is_enabled()
    record('History reset/restore preserves completion ticks')
    page.goto(base+'?view=practice');page.locator('#checkAnswer').click();assert page.locator('#modelAnswer').is_hidden();page.locator('#response').fill('It takes place in Thailand.');page.locator('#checkAnswer').click();assert page.locator('#modelAnswer').is_visible()
    record('Practice model answer follows a student attempt')
    context.close()
    ...
    record('Blocked browser storage does not break learning')
    browser.close()
