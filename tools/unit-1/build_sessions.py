...
OUT = ROOT / 'grade7/unit-1'
E = html.escape
VERSION = '20260922-unit1-dates1'
EXAM = '2026-10-22'
...
def shell(title, body):
    return '<!doctype html><html lang="en" dir="ltr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+E(title)+' | Unit 1</title><link rel="stylesheet" href="unit.css"><link rel="stylesheet" href="meetings.css?v='+VERSION+'"></head><body><main><header class="unit-header"><a class="home" href="../" aria-label="Grade 7">⌂</a><a href="./">Unit 1 · Meetings</a></header>'+body+'</main><script src="meetings.js?v='+VERSION+'"></script></body></html>'
...
def ranges(words, word_slides):
    ...
    for w in words:
        ...
        if not grouped or grouped[-1]['group']!=w['group']:
            grouped.append(dict(group=w['group'],start=n,end=n,href='../band2-groups-01-02/full.html#slide-'+str(word_slides[w['id']])))
...
def build():
    ...
    words=json.loads((OUT/'../band2-groups-01-02/entries.json').read_text())
    ...
    for i,base in enumerate(MEETINGS):
        ...
        m['timing']=[(14,'Vocabulary: recall, introduce the assigned batch and use four to six entries'),(24,'Reading or listening: find evidence and compare'),(14,'Grammar: model, try and check'),(8,'Independent writing or speaking'),(4,'Exit check and home-review plan')]
        m['activities']=[
            activity(mid+'-vocab','Vocabulary · '+str(m['count'])+' entries',14,'Work through the stated ranges. Recall earlier words, then focus on four to six examples for active use. Review every assigned entry at home.',[(g['title'],g['href']) for g in m['ranges']]),
            activity(mid+'-reading','Read, listen and compare',24,m['reading_task'],m['reading']),
        ...
        m['homeActivities']=[]
        for n in range(3):
            size,extra_count=divmod(len(batch),3);start=n*size+min(n,extra_count);end=start+size+(n<extra_count);part=batch[start:end];rr=ranges(part,word_slides)
            absolute=offset+start+1;last=absolute+len(part)-1
            extra,extra_href=m['home'][n]
            instruction=f'Review these {len(part)} entries: listen, cover the meaning and recall it. Spend 7 minutes on these words and 8 minutes on the task below. '+extra
            links=[(g['title'],g['href']) for g in rr]+[('Review task',extra_href)]
    ...
    assert offset==110 and len(set(w for m in meetings for w in m['vocabularyIds']))==110
    (OUT/'meetings.json').write_text(json.dumps(meetings,ensure_ascii=False,indent=2)+'\n')
    total=len(meetings)
    body='<div data-meeting-index data-dated-plan><p class="eyebrow">Grade 7 · Unit 1</p><h1>Meetings by date</h1><p class="unit-title">The Same Way</p><p>Four remaining double meetings, 80 minutes each. Each meeting has 64 active minutes and 16 minutes for transitions and interruptions.</p><p class="schedule-note">Next exam: <time datetime="'+EXAM+'">'+label(EXAM)+'</time>. No school on 4 October. The exam day is excluded. The two earlier shortened meetings are not counted here.</p>'
    ...
    for m in meetings:
        ...
        slides.append(dict(title=prefix+m['title'],body='<p><strong>Goal:</strong> '+E(m['goal'])+'</p>'+skills(m)+'<p><strong>Vocabulary:</strong> '+E(ranges_text)+'. '+str(m['count'])+' entries. Review familiar entries and introduce unfamiliar ones in 14 minutes; focus on four to six entries for active use. All entries return in three home rounds.</p><p class="mini">Before class: '+E(' '.join(m['prep']))+'</p><p class="mini"><a href="./#'+m['id']+'">Dated classwork and home links</a></p>'))
        timing='<ol class="timing-list">'+''.join('<li><strong>'+str(minutes)+' min</strong> · '+E(task)+'</li>' for minutes,task in m['timing'])+'</ol>'
        slides.append(dict(title=prefix+'64-minute teaching sequence',body=timing+'<p class="mini">Scheduled double meeting: 80 minutes. Keep 16 minutes for transitions and interruptions. The writing block includes an independent attempt before feedback. Use the indexed grammar examples; remaining practice returns at home.</p>'))
        slides.append(dict(title=prefix+'Check and review at home',body='<p><strong>Check:</strong> '+E(m['check'])+'</p><p><strong>Support:</strong> '+E(m['support'])+'</p><p><strong>Exit:</strong> '+E(m['exit'])+'</p><p><strong>At home:</strong> three 15-minute rounds; vocabulary batches of '+', '.join(str(len(a['vocabularyIds'])) for a in m['homeActivities'])+' entries, with linked reading, grammar and writing. Due before '+label(m['homeDue'])+'.</p><p class="mini"><a href="student-preparation.html#'+m['id']+'">Exact home-review links</a> · <a href="teacher.html">Answer keys and teaching notes</a></p>'))
    (OUT/'meeting-guide-data.js').write_text('const TEACHER = '+json.dumps(slides,ensure_ascii=False)+';\n')
    template=(ROOT/'grade7/unit-2/meeting-guide.html').read_text().replace('Unit 2','Unit 1').replace('20260922-dates1',VERSION)
    (OUT/'meeting-guide.html').write_text(template)
    engine=(ROOT/'grade7/unit-2/meeting-guide.js').read_text().replace('Unit 2','Unit 1')
