...
(async()=>{
 ...
 for(const [width,height] of [[1366,900],[390,844],[360,640]]){
  ...
  assert(await p.locator('[data-meeting]').count()===4,'Four dated meeting sections');await visit(new URL('resources.html',base).href);
  ...
  await visit(new URL('reading/?text=boat',base).href);
  for(let t=0;t<(width===390?4:1);t++){
