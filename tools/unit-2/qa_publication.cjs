...
(async()=>{
 ...
 report.base=base;const browser=await chromium.launch({headless:true,executablePath:process.env.UNIT2_CHROMIUM,args:['--no-sandbox']});
 const assetContext=await browser.newContext();
 const assets=fs.readdirSync(unit,{recursive:true}).filter(f=>/\.(mp3|pdf|jpg)$/.test(f));
 ...
 for(const [width,height] of [[1366,900],[390,844],[360,640]]){
  const ctx=await browser.newContext({viewport:{width,height},isMobile:width<700,hasTouch:true,permissions:['clipboard-read','clipboard-write']});const p=await ctx.newPage();
  ...
  await p.goto(new URL('../',base).href);assert(await p.locator('a.card').first().getAttribute('href')==='unit-2/','Unit 2 is first on Grade 7 page');
  ...
  for(const f of [...links.filter(f=>!f.endsWith('.pdf')),'workbook-key.html']){
   await p.goto(new URL(f,base).href);await p.evaluate(()=>document.fonts.ready);
  ...
  for(const f of ['grammar-a/','grammar-b/']){await p.goto(new URL(f,base).href);const before=await p.locator('#counter').innerText();await p.keyboard.press('ArrowRight');assert(await p.locator('#counter').innerText()!==before,`Grammar navigation ${f}`);}
  await p.goto(new URL('companion-stories.html',base).href);await p.evaluate(async()=>{for(const im of document.images){im.loading='eager';await im.decode();}});
  await p.goto(new URL('reading/?text=snow-day',base).href);
  ...
  await p.goto(new URL('vocabulary/full.html',base).href);await p.waitForFunction(()=>!document.querySelector('#sound').disabled);
  ...
  await p.goto(base);await p.screenshot({path:`/tmp/unit2-release-${process.env.UNIT2_BASE_URL?'public':'local'}-${width}.png`,fullPage:true});
