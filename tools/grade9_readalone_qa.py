...
import functools, hashlib, http.server, json, os, sys, threading, time
...
def ready(page):page.wait_for_function('window.TeachersReadAlone?.data && window.TeachersReadAlone?.timing && document.querySelector("#raText")',{ 'timeout':45000} if False else None,timeout=45000)
...
try:
 with sync_playwright() as p:
  ...
  for width,height in [(1440,900),(390,844),(360,640)]:
   ...
   for mode,total in [('text',len(data['pages'])),('sentences',92)]:
    for n in range(total):
     page.evaluate('([n,m])=>TeachersReadAlone.setPage(n,m)',[n,mode]);page.wait_for_timeout(30)
     ...
     else:check(f'sentence preserved {width} step {n+1}',page.locator('#raText').text_content()==data['sentences'][n//2]['en'])
   ...
   # The original deck must preserve 96 original slides and contain 46 added reveal slides.
   ...
   for slide_id in slide_ids:
    page.evaluate('(id)=>location.hash=id',slide_id);page.wait_for_timeout(20)
   ...
   context.close()
  # Playback uses the real recording, not mocked Audio objects.
  ...
  page.wait_for_function('document.querySelector(".ra-spoken")',timeout=8000);check('yellow word tracks audio',page.evaluate('(()=>{let a=TeachersReadAlone.audio,t=TeachersReadAlone.clip.words.find(w=>a.currentTime>=w.start&&a.currentTime<w.end),n=document.querySelector(".ra-spoken");return !!t&&n?.dataset.w===String(t.index)})()'))
  ...
  page.locator('.ra-mode[data-mode="text"]').click();page.locator('[data-ra-play]').click();page.evaluate('TeachersReadAlone.audio.currentTime=TeachersReadAlone.clip.end-.04');page.wait_for_timeout(800);check('stops at passage boundary',page.evaluate('TeachersReadAlone.audio.paused'));check('no automatic passage advance',page.evaluate('TeachersReadAlone.page')==data['pages'].index(data['pages'][page.evaluate('TeachersReadAlone.page')]))
  page.locator('#raNext').click();page.wait_for_timeout(300);check('continue starts next passage',not page.evaluate('TeachersReadAlone.audio.paused'))
